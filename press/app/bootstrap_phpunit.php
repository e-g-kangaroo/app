...
$common_path    = rtrim($_SERVER['common_path'], '/').'/';
...
( ! is_dir($common_path) and is_dir(DOCROOT.$common_path)) and $common_path = DOCROOT.$common_path;

define('APPPATH', realpath($app_path).DIRECTORY_SEPARATOR);
define('PRESSPATH', realpath($app_path).DIRECTORY_SEPARATOR);
define('COMMONPATH', realpath($common_path).DIRECTORY_SEPARATOR);
